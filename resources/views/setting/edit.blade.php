...
<html lang="en">
  ...
  <body>
    <div class="container mt-5">
        <center><h3 style="background: lightblue">Edit Setting</h3></center>
        <form action="{{ route('setting.update', $setting->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="mb-3">
              <label class="form-label">Name</label>
              <input value="{{ old('name', $setting->name) }}" name="name" type="text" class="form-control @error('name') is-invalid @enderror">
                @error('name')
            ...
            <center>
                <input value="Update Data" type="submit" style="background-color: darkgrey;border-radius:15px;width:400px;"></br>
            </center>
          </form>
    </div>
